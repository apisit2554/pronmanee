...
function savePosition($pos_name){
	...
	echo ("<script language='JavaScript'>
		window.location.href='manage_position.php';
		</script>"); 
...
function saveStartWork($users_id,$projects_id,$work_start_date,$work_start_time){
	...
	mysqli_close($con);
	echo ("<script language='JavaScript'>
		window.location.href='edit_schedule.php?id=$users_id';
		</script>"); 
...
function saveIncomeExpense($projects_id,$ie_category,$ie_detail,$ie_date,$ie_time,$ie_type,$ie_amount){
	...
	mysqli_close($con);
	echo ("<script language='JavaScript'>
		window.location.href='manage_income_expense.php';
		</script>"); 
...
function uploadImagePeriod($period_id,$projects_id,$period_image,$total){
	...
	/*if($period_image != null ){
		if(move_uploaded_file($_FILES["period_image"]["tmp_name"],"images/period/".$_FILES["period_image"]["name"]))
		{
			$sql="UPDATE projects_period SET period_image='".$_FILES["period_image"]["name"]."' WHERE id = '".$period_id."'";
		}
	}*/
	$yThai = date("Y")+543;
	$dateNow = $yThai.date("-m-d");
...
function editJobLast($job_last_img,$total){

	global $con;
	
	
	mysqli_query($con,"DELETE * FROM job_lasted");
		for( $i=0 ; $i < $total ; $i++ ) {
			$tmpFilePath = $_FILES['job_last_img']['tmp_name'][$i];
			if ($tmpFilePath != ""){
				$newFilePath = "images/job_last/" . $_FILES['job_last_img']['name'][$i];
				if(move_uploaded_file($tmpFilePath, $newFilePath)) {
					$sql_detail = "INSERT INTO job_lasted (job_last_img) VALUES ('".$_FILES['job_last_img']['name'][$i]."')";
					mysqli_query($con,$sql_detail);
	...
	echo ("<script language='JavaScript'>
		...
		window.location.href='job_last.php';
		</script>"); 
	
}

function getAllImageLastjob(){
	global $con;

	$sql = "SELECT * FROM job_lasted ORDER BY id ASC";
	$res = mysqli_query($con,$sql);
	$data = array();
	while($row = mysqli_fetch_assoc($res)) {
		$namesArray[] = array(
			'id' => $row['id'],
			'job_last_img' => $row['job_last_img']);
	}
...
function saveWithDraw($users_id,$wage,$withdraw_amount){
	...
	mysqli_close($con);
	echo ("<script language='JavaScript'>
		window.location.href='detail_salary.php?id=$users_id';
		</script>");
}
?>